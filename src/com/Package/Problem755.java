...
public class Problem755 {
    ...
    public void run(){
        ...
        for(long i = 1; i < 1001; i++) {
            int temp = countRep(i, fibonacci.length - 1);
            sum += temp;
            System.out.println("f(" + i + ") = " + temp + " , sum = " + sum);
        }
    }

    public int countRep(long num, int i){
        if(num < 0)
            return 0;
        if(num == 0)
            return 1;
        if (i < 0)
            return 0;
        return countRep(num - fibonacci[i], i-1) + countRep(num,i-1);
    }
